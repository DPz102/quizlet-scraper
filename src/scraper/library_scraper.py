...
class LibraryScraper(BaseScraper):
    ...
    def get_user_sets(self) -> List[FlashCardSet]:
        ...
        page: Page = self._navigate(self.QUIZLET_LATEST_URL)
        
        # Scroll to load all sets
        self._scroll_to_bottom(page)
        
        sets: List[FlashCardSet] = self._extract_sets_from_page(page)
    ...
    def get_shared_sets(self) -> List[FlashCardSet]:
        ...
        classes: List[str] = self._get_user_classes()
        for class_url in classes:
            try:
                class_sets: List[FlashCardSet] = self.get_class_sets(class_url)
    ...
    def get_class_sets(self, class_url: str) -> List[FlashCardSet]:
        ...
        page: Page = self._navigate(class_url)
        
        # Click on "Sets" tab if present
        try:
            sets_tab: Locator = page.locator('a:has-text("Sets"), button:has-text("Sets")').first
    ...
    def get_folder_sets(self, folder_url: str) -> List[FlashCardSet]:
        ...
        page: Page = self._navigate(folder_url)
    ...
    def _get_user_classes(self) -> List[str]:
        ...
        page: Page = self._navigate(f"{self.QUIZLET_BASE_URL}/latest")
        ...
        for selector in class_selectors:
            try:
                links: List[Locator] = page.locator(selector).all()
                for link in links:
                    href: str | None = link.get_attribute("href")
                    if href and "/class/" in href:
                        full_url: str = href if href.startswith("http") else f"{self.QUIZLET_BASE_URL}{href}"
    ...
    def _extract_sets_from_page(self, page: Page) -> List[FlashCardSet]:
        """
            ...
            try:
                ...
                for link in links:
                    try:
                        ...
                        set_id: str | None = self._extract_set_id(full_url)
                        if not set_id:
                            continue
                        
                        # Get title
                        title: str = self._extract_title(link)
                        
                        # Get term count if available
                        term_count: int = self._extract_term_count(link)
    ...
    def _extract_title(self, link: Locator) -> str:
        ...
        try:
            ...
            text: str = link.inner_text()
            if text:
                return text.strip()
            
            # Try aria-label
            label: str | None = link.get_attribute("aria-label")
            if label:
                return label.strip()
            
            # Try title attribute
            title: str | None = link.get_attribute("title")
            if title:
                return title.strip()
    ...
    def _extract_term_count(self, link: Locator) -> int:
        ...
        try:
            ...
            parent: Locator = link.locator("..")
            text: str = parent.inner_text()
            
            # Look for patterns like "50 terms" or "50 thuật ngữ"
            match: re.Match[str] | None = re.search(r'(\d+)\s*(?:terms?|thuật ngữ|cards?|thẻ)', text, re.IGNORECASE)
            if match:
